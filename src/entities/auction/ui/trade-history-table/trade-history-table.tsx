...
import type { TradeHistory } from '../../model/types';

interface TradeHistoryTableProps {
  items: TradeHistory[];
  onReceivePayment?: (transactionId: number) => void;
}

export const TradeHistoryTable = ({ items, onReceivePayment }: TradeHistoryTableProps) => {
  return (
    <div className="bg-[#1f1f24] border border-[#2a2a32] rounded-2xl overflow-hidden">
      ...
      <div className="divide-y divide-[#2a2a32]">
        {items.map((item) => (
          <div key={item.transactionId} className="grid grid-cols-6 gap-4 p-4 items-center text-gray-200">
            ...
            <div className={`text-sm font-medium ${getStatusColor(item.type, item.status)}`}>
            ...
            <div>
              {item.status === 'PROCESSING' && (
                <button
                  onClick={() => onReceivePayment?.(item.transactionId)}
                  className="px-3 py-1 text-sm text-blue-400 border border-blue-400 hover:bg-blue-400 hover:text-white rounded transition"
                >
                  수령
                </button>
              )}
              {item.type === 'SELL' && item.status === 'COMPLETED' && (
                <span className="text-green-400 text-sm">판매 완료</span>
              )}
              {item.type === 'BUY' && item.status === 'COMPLETED' && (
                <span className="text-blue-400 text-sm">구매 완료</span>
              )}
...
function getStatusColor(type: string, status: string): string {
  if (status === 'COMPLETED') {
    return type === 'SELL' ? 'text-green-400' : 'text-blue-400';
  }
  switch (status) {
    case 'PROCESSING':
